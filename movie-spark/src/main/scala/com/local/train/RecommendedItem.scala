...
import com.local.caseclass.{ItemPref, ItemSimi, UserRecommend}
import org.apache.spark.rdd.RDD

/**
  * 用户推荐计算
  * 根据物品相似度、用户评分、指定最大推荐数进行用户推荐
  *
  * @author Michael Chu
  * @since 2020-03-18 11:42
  */
class RecommendedItem {

  def Recommend(items_similar: RDD[ItemSimi],
                user_prefer: RDD[ItemPref],
                r_number: Int): RDD[UserRecommend] = {
    //   0 数据准备
    ...
    rdd_app1_R8.map(f => UserRecommend(f._1, f._2, f._3))
  }
  ...
  /**
    ...
    */
  def Recommend(items_similar: RDD[ItemSimi],
                user_prefer: RDD[ItemPref]): RDD[UserRecommend] = {
    //   0 数据准备
    val rdd_app1_R1 = items_similar.map(f => (f.itemid1, f.itemid2, f.similar))
    val user_prefer1 = user_prefer.map(f => (f.userId, f.itemId, f.pref))
    //   1 矩阵计算――i行与j列join
    ...
    val rdd_app1_R6 = rdd_app1_R5.map(f => (f._1, f._2._1, f._2._2)).
      sortBy(f => (f._1, f._3))
    rdd_app1_R6.map(f => UserRecommend(f._1, f._2, f._3))
  }
